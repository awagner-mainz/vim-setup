...
if s:other_style==0 | let g:colors_name = s:color_name
  ...
  execute "autocmd TermChanged * if g:colors_name == '".s:color_name."' | " . "colo ".s:color_name." | endif"
else
  execute "runtime colors/".s:style.".vim"
endif

" Command to go different schemes easier.
execute "command! -nargs=1 Colo if '".s:color_name."'!=\"<args>\" | " 'let g:psc_style = "<args>"| endif | colo '.s:color_name
